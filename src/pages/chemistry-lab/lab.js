...
class VirtualLab {
    ...
    async addSample() {
        ...
        await this.animateFlaskFill(30, 'rgba(224, 242, 254, 0.9)');

        // Sync 3D
        this.call3D('animateBottle', 'sample');
        this.call3D('fillFlask', 30, 0xe0f2fe);
    ...
    async addBuffer() {
        ...
        await this.animateFlaskFill(40, 'rgba(224, 242, 254, 0.9)');

        // Sync 3D
        this.call3D('animateBottle', 'buffer');
        this.call3D('fillFlask', 40);
    ...
    exportPDF() {
        ...
        const procedure = this.reportProcedureInput.value.trim() || 'Pipette 25mL water sample into flask, add buffer and indicator, titrate with EDTA until blue endpoint.';
        const result = this.reportResultInput.value.trim() || `The hardness of the given water sample is ${this.state.calculatedHardness.toFixed(1)} mg/L as CaCO₃.`;
        ...
        doc.autoTable({
            ...
            body: [
                ...
                ['2', 'Final Burette Reading (mL)', this.state.titration1Volume.toFixed(2), this.state.titration2Volume.toFixed(2)],
                ['3', 'Volume of EDTA Used (mL)', this.state.titration1Volume.toFixed(2), this.state.titration2Volume.toFixed(2)],
                ['4', 'Endpoint Color', 'Wine Red → Blue', 'Wine Red → Blue']
            ],
            theme: 'grid',
        ...
        doc.setTextColor(0);
        doc.setFont(undefined, 'bold');
        doc.text(`Average Volume of EDTA = (${this.state.titration1Volume.toFixed(2)} + ${this.state.titration2Volume.toFixed(2)}) / 2 = ${this.state.averageVolume.toFixed(2)} mL`, 25, y);
        ...
        y += 8;
        doc.text(`Where: V_EDTA = ${this.state.averageVolume.toFixed(2)} mL, M = ${this.state.edtaConcentration} M, V_sample = ${this.state.sampleVolume} mL`, 25, y);
        y += 8;
        doc.text(`Substitution: = (${this.state.averageVolume.toFixed(2)} × ${this.state.edtaConcentration} × 100000) / ${this.state.sampleVolume}`, 25, y);
        y += 10;
        doc.setFontSize(12);
        doc.setFont(undefined, 'bold');
        doc.text(`Hardness = ${this.state.calculatedHardness.toFixed(2)} mg/L as CaCO₃`, 25, y);
        ...
        y += 4;
        doc.text(`Expected Hardness: ${this.state.targetHardness} ppm  |  Accuracy: ${this.state.accuracy.toFixed(1)}%`, 25, y);
