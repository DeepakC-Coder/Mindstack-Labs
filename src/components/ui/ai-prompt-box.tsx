 ...
 const VoiceRecorder: React.FC<VoiceRecorderProps> = ({
  ...
  const recognitionRef = React.useRef<any>(null);
   ...
   React.useEffect(() => {
     if (isRecording) {
      ...
      // Initialize Web Speech API for realtime transcription
      ...
      onStopRecording(time, transcript);
      setTranscript("");
       setTime(0);
     }
     return () => {
  ...
  }, [isRecording, time, transcript, onStartRecording, onStopRecording]);
   ...
   return (
     ...
     >
      ...
      {transcript && (
        <div className="w-full px-4 mb-2 text-sm text-white/70 text-center max-h-16 overflow-y-auto">
          {transcript}
        </div>
      )}
 ...
 export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
    ...
    const enabled = {
      ...
      think: features?.think ?? true,
      canvas: features?.canvas ?? true,
    };
   ...
   React.useEffect(() => {
    ...
    const handleSubmit = () => {
     if (input.trim() || files.length > 0) {
       let messagePrefix = "";
        if (enabled.webSearch && showSearch) messagePrefix = "[Search: ";
        else if (enabled.think && showThink) messagePrefix = "[Think: ";
        else if (enabled.canvas && showCanvas) messagePrefix = "[Canvas: ";
  ...
  const handleStopRecording = (duration: number, transcript?: string) => {
      console.log(`Stopped recording after ${duration} seconds`);
      setIsRecording(false);
     // If we have a transcript from speech recognition, set it in the input field (don't send)
     if (transcript && transcript.trim()) {
       setInput((prev) => prev ? `${prev} ${transcript.trim()}` : transcript.trim());
     }
     // Don't auto-send - let user review and edit the transcribed text
    };
     ...
     <>
       ...
       >
         ...
         >
            <PromptInputTextarea
              placeholder={
                enabled.webSearch && showSearch
                  ? "Search the web..."
                  : enabled.think && showThink
                  ? "Think deeply..."
                  : enabled.canvas && showCanvas
                  ? "Create on canvas..."
                  : placeholder
              }
             className="text-base"
         ...
         <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
           ...
           >
              ...
              {(enabled.webSearch || enabled.think || enabled.canvas) && (
                <div className="flex items-center">
                  ...
                  {enabled.webSearch && enabled.think && <CustomDivider />}
                  ...
                  {(enabled.canvas && (enabled.webSearch || enabled.think)) && <CustomDivider />}
